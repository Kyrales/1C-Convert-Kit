...
**Тест: test_server_ib_dt_roundtrip**
- Выгрузка тестовой серверной ИБ в DT через `ib2dt`
- Загрузка отличающейся пустой конфигурации и подтверждение изменения базы
- Восстановление полученного DT поверх той же ИБ через `dt2ib` с `V8_IB_UPDATE=1`
- Проверка восстановления по generation ID конфигурации (CF после применения может бинарно отличаться)
- IBCMD проверяется всегда; DESIGNER дополнительно проверяется при настроенном `V8_IB_USER`
- Варианты выполняются последовательно в одном тесте, аварийное восстановление выполняется через IBCMD
- Требует явного разрешения: `RUN_DESTRUCTIVE_1C_TESTS=1`
- Использует демонстрационную базу из `projects/Demo/ibcmd/Демо_cf_в_server_ib_ibcmd`
