...
class TestValidationConverter:
    ...
    def test_get_output_extension(self):
        ...
        assert converter.get_output_extension() == ''
    ...
    def test_validate_non_edt_source(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env_vars = {
                ...
                'V8_DST_PATH': ''  # Не требуется для валидации
            }
            converter = ValidationConverter(env_vars, silent=True)
            ...
            assert 'EDT проект' in str(exc_info.value)
...
class TestValidationConverterStructureCheck:
    ...
    def test_validate_structure_missing_config_and_extension_xml(self):
        """Тест проверки структуры без Configuration.xml и Extension.xml."""
        import tempfile
        with tempfile.TemporaryDirectory() as tmpdir:
            src_path = Path(tmpdir) / 'edt_project'
            src_path.mkdir()
            ...
            assert 'Configuration.xml' in error_msg or 'Extension.xml' in error_msg
    
    def test_validate_structure_success_with_configuration(self):
        """Тест успешной проверки структуры с Configuration.xml."""
        import tempfile
        with tempfile.TemporaryDirectory() as tmpdir:
            src_path = Path(tmpdir) / 'edt_project'
            src_path.mkdir()
    ...
    def test_validate_structure_with_src_directory(self):
        """Тест проверки структуры с директорией src."""
        import tempfile
        with tempfile.TemporaryDirectory() as tmpdir:
            src_path = Path(tmpdir) / 'edt_project'
            src_path.mkdir()
