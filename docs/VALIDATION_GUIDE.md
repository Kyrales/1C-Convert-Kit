...
`ValidationConverter` проверяет структуру EDT-проекта, запускает валидацию через `1cedtcli` или `ring` и сохраняет отчет в формате TSV.
...
2. Убедитесь, что инструмент EDT работает корректно
...
- [Руководство по архитектуре](../README.md#architecture)
